...
class BattleshipGame:
    ...
    def save_game_state(self, filename="battleship_state.json"):
        ...
        print(f"Estado do jogo salvo em {filename}.")

    def load_game_state(self, filename="battleship_state.json"):
        """Carrega o estado do jogo de um arquivo JSON."""
        if os.path.exists(filename):
            with open(filename, "r") as file:
                state = json.load(file)
            self.my_board = state["my_board"]
            self.opponent_board = state["opponent_board"]
            self.turn = state["turn"]
            print(f"Estado do jogo carregado de {filename}.")
        else:
            print(f"Arquivo {filename} não encontrado. Começando novo jogo.")
    ...
    def handle_turn(self):
        while self.running:
            self.display_boards()  # Mostrar os tabuleiros atualizados
            if self.turn:
    ...
    def start(self):
        self.load_game_state()  # Carregar estado do jogo, se existir
        self.place_ships()
        self.display_boards()
        ...
        if mode == "s":
            ...
            self.start_client()
        self.save_game_state()  # Salvar estado do jogo ao encerrar
